...
<script type="text/javascript">
	...
	function showWarehouseGrid() {
		...
		$("#warehouseDetailGrid").jqGrid({
			colNames : [ '품목코드' , '품목명' , '재고량', '안전재고량' ],
			colModel : [
				{ name : 'itemCode', width:100, align : "center",editable:false},
				{ name : 'itemName', width:100, align : "center", editable:false},
				{ name : 'stockAmount', width:100, align : "center",editable:false},
				{ name : 'safetyAllowanceAmount', width:100, align : "center",editable:false},
			],
			width : 450,
			height : 300,
			caption : "창고세부정보",
		...
		$.ajax({
			...
			success : function(dataSet) {
				console.log(dataSet.gridRowJson);	//여기에 한줄 없으면 바로 실행이 안됨...왜 그럼?ㅋㅋㅋㅋ
				var gridRowJson = dataSet.gridRowJson;
				// 그리드 초기화
				$('#warehouseDetailGrid').jqGrid('clearGridData');	
...
<body>
	<div id="tabs">
		...
		<div id="warehouseDialog">
			<table id="warehouseDetailGrid"></table>
		</div>
