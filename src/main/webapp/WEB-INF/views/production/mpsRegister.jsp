...
<article class="contract">
    <div class="contract__Title">
        ...
        <button id="registerNewMpsButton">MPS등록</button>
        <button id="mpsModalBtn">MPS조회</button>
    </div>
</article>
<article class="contractMpsGrid">
    <div align="center">
        <div id="contractMpsGrid" class="ag-theme-balham" style="height:30vh; width:auto; text-align: center;"></div>
    </div>
</article>
<div>
    <h5>📷 판매 계획</h5>
...
<article class="salesMpsGrid">
    <div align="center" class="ss">
        <div id="salesMpsGrid" class="ag-theme-balham" style="height:30vh;width:auto;"></div>
    </div>
</article>
...
<script>
  const contractMpsGrid = document.querySelector("#contractMpsGrid");
  const salesMpsGrid = document.querySelector("#salesMpsGrid");
  ...
  contractCandidateSearchBtn.addEventListener("click", () => {
    ...
    let xhr = new XMLHttpRequest();
      xhr.open('POST', "${pageContext.request.contextPath}/production/searchContractDetailListInMpsAvailable.do"
          + "?method=searchContractDetailListInMpsAvailable"
          + "&searchCondition=" + searchCondition
          + "&startDate=" + startDate
          + "&endDate=" + endDate
          +"&includeMrpApply=" + "includeMrpApply", // o MRP 적용된 주생산계획도 검색
          true);
    ...
    Swal.fire({
      ...
      xhr.onreadystatechange = () => {
        if (xhr.readyState === 4 && xhr.status === 200) {
          ...
          // 초기화
          let step;
          for (step = 0; step < resultRows.length; step++) {
  ...
  // O Select MPS
  mpsModalBtn.addEventListener("click", () => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    new agGrid.Grid(contractMpsGrid, mpsGridOptions);
    // new agGrid.Grid(salesMpsGrid, estDetailGridOptions);
  })
</script>
</body>
</html>